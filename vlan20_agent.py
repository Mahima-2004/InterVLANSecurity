#!/usr/bin/env python3
"""
vlan20_agent.py - Enhanced VLAN20 (Finance) Agent for Distributed Inter-VLAN Monitoring
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('vlan20_agent.log'),
        logging.StreamHandler()
    ]
)

class EnhancedVLAN20Agent:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Enhanced VLAN20 Agent - Finance Department")
    ...
    def setup_gui(self):
        ...
        # Title
        title = tk.Label(main_frame, text="Enhanced VLAN20 Agent - Finance Department", 
    ...
    def create_file_transfer_tab(self, notebook):
        ...
        self.dest_vlan_var = tk.StringVar(value="VLAN10")
        ttk.Radiobutton(dest_frame, text="VLAN10 (HR)", variable=self.dest_vlan_var, 
                       value="VLAN10").pack(side=tk.LEFT, padx=10)
    ...
    def create_message_transfer_tab(self, notebook):
        ...
        self.msg_dest_var = tk.StringVar(value="VLAN10")
        ttk.Radiobutton(send_frame, text="VLAN10 (HR)", variable=self.msg_dest_var, 
                       value="VLAN10").pack(side=tk.LEFT, padx=10)
    ...
    def export_transfer_history(self):
        fname = f"vlan20_transfer_history_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        with open(fname, 'w') as f:
            f.write("VLAN20 Transfer History Export\n")
            f.write("=" * 50 + "\n\n")
    ...
    def export_logs(self):
        fname = f"vlan20_logs_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        with open(fname, 'w') as f:
    ...
    def export_messages(self):
        ...
        fname = f"vlan20_messages_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        with open(fname, 'w', encoding='utf-8') as f:
            f.write("VLAN20 (Finance) - Received Messages\n")
            f.write("=" * 50 + "\n\n")
            for msg in self.received_messages:
                timestamp = msg.get('timestamp', 'Unknown')
                sender = msg.get('sender', 'Unknown')
    ...
    def export_received_files(self):
        ...
        fname = f"vlan20_received_files_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        with open(fname, 'w', encoding='utf-8') as f:
            f.write("VLAN20 (Finance) - Received Files\n")
            f.write("=" * 50 + "\n\n")
